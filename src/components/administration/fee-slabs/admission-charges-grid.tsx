...
const getColumns = (brandColor?: string): ColumnDef<AdmissionCharge>[] => [
  ...
  },
  {
    ...
    header: "Charge Name",
    cell: ({ getValue }) => <span className="font-medium text-slate-900 dark:text-slate-100">{getValue<string>()}</span>,
  },
  {
  ...
  },
  {
    ...
    cell: ({ getValue }) => {
      ...
      return (
        <Badge variant={recurring ? "default" : "secondary"} className="text-[10px] uppercase tracking-wider">
          {recurring ? "Yes" : "No"}
        </Badge>
      );
    },
